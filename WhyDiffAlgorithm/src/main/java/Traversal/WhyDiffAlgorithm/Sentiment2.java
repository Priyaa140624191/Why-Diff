...
public class Sentiment2 {
	...
	public static void main(String[] args) {
		...
		try (Transaction tx = graphDB.beginTx()) {
			...
			for (int i = 0; i < divergingNodeLists.size(); i++) {
				if (!divergingNodeLists.get(i).toString().contains("ABSENT")) {
					...
					for (int j = 0; j < divergingNodes.size(); j++) {
						String NodeProp = "";
						System.out
								.print(divergingNodeLists.get(i).get(j) + "|");
						resultString = resultString
								+ divergingNodeLists.get(i).get(j) + "|";
						if (!divergingNodeLists.get(i).get(j).toString()
								.contains("ABSENT")) {
							if (divergingNodeLists.get(i).get(j).toString()
									.contains("block")) {
								Node activityNode = graphDB.findNode(
										NodeType1.activity, "id",
										divergingNodeLists.get(i).get(j));
								NodeProp = activityNode.getProperty(
										"properties").toString();
							} else {
								Node entityNode = graphDB.findNode(
										NodeType1.entity, "id",
										divergingNodeLists.get(i).get(j));
								NodeProp = entityNode.getProperty("attributes")
										.toString();
							}
						}
						System.out.print("\n|-->Properties: "+NodeProp + "\n");
						...
						if (!divergingNodeLists.get(i).get(j).toString()
							...
							if (divergingNodeLists.get(i).get(j).toString()
								...
								Node entityNode = graphDB.findNode(
										NodeType1.entity, "id",
										divergingNodeLists.get(i).get(j));
								NodeProp = "\n|-->Properties: "+entityNode.getProperty("attributes")
										.toString();
							}
						}
						System.out.print(NodeProp);
						resultString =  resultString +NodeProp;
			...
			System.out.println("Listing Divergent properties after sync");
			System.out
					.println("|-------------------------------------------------------------------------------------------------------------------------------------------------|");
	...
	private static void makeDeltaGraph() {
		...
		/*
		 * for (int i = 0; i < deltaActList.size(); i++) { String original =
		 * deltaActList.get(i).toString(); String newString =
		 * deltaActList.get(i).toString() +
		...
		/*
		 * for (int i = 0; i < deltaEntList.size(); i++) { String original =
		 * deltaEntList.get(i).toString(); String newString =
		 * deltaEntList.get(i).toString() + "[style=filled][fillcolor = pink]";
			...
			for (int j = 0; j < secondExtra.size(); j++) {
				...
				if (firstExtra.get(i).toString().contains("invocation")
					...
					if (checkActivityEqual(actNode1.getProperty("properties")
						...
						divergingProperties.add(actNode1.getProperty(
								"properties").toString());
						divergingProperties.add(actNode2.getProperty(
								"properties").toString());
				...
				if (!firstExtra.get(i).toString().contains("invocation")
					...
					if (checkFurtherEntityEqual(
						...
						divergingProperties = new ArrayList<String>();
						divergingProperties.add(entNode1.getProperty(
								"attributes").toString());
						divergingProperties.add(entNode2.getProperty(
								"attributes").toString());
						divergingPropertyLists.add(divergingProperties);
					}
				}
		...
		try {
			...
			for (Map<String, Object> row1 : getEntity1) {
				...
				for (Map<String, Object> row2 : getEntity2) {
					 ...
					 * .substring(provLabel1 + 13, provType1 - 6); String
					 * fileLabel2 = firstEntity2.getProperty("attributes")
					 * .toString() .substring(provLabel2 + 14, provType2 - 6);
					 * System.out.println("File Label "+fileLabel1);
					 * System.out.println("File Label "+fileLabel2);
					 */

					boolean isEqual = firstEntity1.getProperty("id").toString() == firstEntity2
							.getProperty("id").toString();

					/*
					 * FileComparison f = new FileComparison(); boolean
					 * isNotEqual = f.fileCompare(fileLabel1, fileLabel2);
					 */
					if (!isEqual) {
						divergingNodes = new ArrayList<String>();
						...
						divergingProperties = new ArrayList<String>();
						divergingProperties.add(firstEntity1.getProperty(
								"attributes").toString());
						divergingProperties.add(firstEntity2.getProperty(
								"attributes").toString());
						divergingPropertyLists.add(divergingProperties);
					}
					break;
			...
			for (String firstActivity : firstActivityList) {
				...
				for (String secondActivity : secondActivityList) {
					...
					if (activityzip1 == true) {
						boolean isEqualact1 = checkActivityEqual(
								firstActivityNode.getProperty("properties")
										.toString(), SecondActivityNode
										.getProperty("properties").toString());
						...
						if (!isEqualact1 && !addedact1) {
							...
							divergingProperties = new ArrayList<String>();
							divergingProperties.add(firstActivityNode
									.getProperty("properties").toString());
							divergingProperties.add(firstActivityNode
									.getProperty("properties").toString());
							divergingPropertyLists.add(divergingProperties);
						...
						for (Node returned2Entity : returnedEntity2List) {
							...
							if (zip == true) {
								...
								if ((!isEqual1) && (!added1)) {
									...
									divergingProperties = new ArrayList<String>();
									divergingProperties.add(returned1Entity
											.getProperty("attributes")
											.toString());
									divergingProperties.add(returned2Entity
											.getProperty("attributes")
											.toString());
									divergingPropertyLists
											.add(divergingProperties);
								}
	...
	private static boolean checkActivityEqual(String ActivityAttributes1,
		...
		System.out.println("Check act properties " + ActivityAttributes1 + " "
				+ ActivityAttributes2);
		try {
